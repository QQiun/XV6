...
void fourteen(char* s)
{
    ...
    if (fd < 0) {
        printf("%s: open 12345678901234/12345678901234/12345678901234 "
               "failed\n",
               s);
        exit(1);
    }
...
// first. inside the bigger usertests binary, we run out of memory first.
...
void sbrkmuch(char* s)
{
    ...
    p = sbrk(amt);
    if (p != a) {
        printf("%s: sbrk test failed to grow big address space; enough "
               "phys mem?\n",
               s);
        exit(1);
    }
